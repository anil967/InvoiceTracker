...
/**
 * GET /api/dept-head/messages - Get Department Head's messages
 */
export async function GET(request) {
    try {
        ...
        const roleCheck = requireRole([ROLES.DEPARTMENT_HEAD])(session.user);
        ...
        console.error('Error fetching dept-head messages:', error);
        return NextResponse.json({ error: 'Failed to fetch messages' }, { status: 500 });
    }
}

/**
 * POST /api/dept-head/messages - Send a message from Department Head
 */
export async function POST(request) {
    try {
        ...
        // Dept Head can message: PMs and Div Heads
        const allowedRecipients = [ROLES.PROJECT_MANAGER, ROLES.DIVISIONAL_HEAD];
        if (!allowedRecipients.includes(recipientRole)) {
            return NextResponse.json({
                error: `Department Head can only message Project Managers or Divisional Heads.`
            }, { status: 403 });
        }
        ...
        await db.createAuditTrailEntry({
            ...
            action: 'MESSAGE_SENT',
            details: `Dept Head message sent to ${recipient.name}: ${subject || '(no subject)'}`
        });
        ...
        console.error('Error sending dept-head message:', error);
        return NextResponse.json({ error: 'Failed to send message' }, { status: 500 });
    }
}

/**
 * PATCH /api/dept-head/messages - Mark messages as read
 */
export async function PATCH(request) {
    try {
        ...
        console.error('Error marking dept-head messages as read:', error);
