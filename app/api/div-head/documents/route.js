...
/**
 * GET /api/div-head/documents - List documents uploaded by divisional head team
 */
export async function GET(request) {
    try {
        ...
        const roleCheck = requireRole([ROLES.ADMIN, ROLES.DIVISIONAL_HEAD])(session.user);
        ...
        // Allow filtering by uploader role (e.g. show only div-head's own uploaded docs)
        if (uploadedByRole) {
        ...
        console.error('Error fetching div-head documents:', error);
        return NextResponse.json({ error: 'Failed to fetch documents' }, { status: 500 });
    }
}

/**
 * POST /api/div-head/documents - Upload document (Divisional Head team)
 */
export async function POST(request) {
    ...
    try {
        ...
        await db.createAuditTrailEntry({
            ...
            details: `Div Head uploaded ${type}: ${file.name}`
        });

        return NextResponse.json({ success: true, document: document.toObject() }, { status: 201 });
    } catch (error) {
        console.error('Error uploading div-head document:', error);
