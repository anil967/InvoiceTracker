...
export default function DeptHeadMessagesPage() {
    const { user } = useAuth();
    ...
    const fetchRecipients = async () => {
        try {
            const res = await fetch('/api/dept-head/messages/recipients', { cache: 'no-store' });
            if (res.ok) {
    ...
    const fetchMessages = async () => {
        try {
            setLoading(true);
            const res = await fetch(`/api/dept-head/messages?type=${activeTab}`, { cache: 'no-store' });
            const data = await res.json();
    ...
    const handleSendMessage = async (e) => {
        e.preventDefault();
        try {
            const res = await fetch('/api/dept-head/messages', {
                method: 'POST',
    ...
    const handleMarkAsRead = async (messageId) => {
        try {
            await fetch('/api/dept-head/messages', {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
    ...
    const divHeads = recipients.filter(r => r.type === 'DivHead');
    const pms = recipients.filter(r => r.type === 'PM');

    return (
        <div className="pb-10">
            <PageHeader
                title="Messages"
                subtitle="Communicate with Divisional Heads"
                icon="Mail"
                accent="blue"
            />
            ...
            <div className="max-w-5xl mx-auto space-y-6 px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                    <div className="flex bg-slate-100 p-1 rounded-2xl w-full sm:w-auto">
                        ...
                        >
                            Inbox {unreadCount > 0 && <span className="ml-1.5 px-1.5 py-0.5 bg-blue-600 text-white text-[8px] rounded-lg">{unreadCount}</span>}
                        </button>
                        <button
                    ...
                    <button
                        ...
                        className="w-full sm:w-auto px-8 py-3 bg-linear-to-br from-blue-600 to-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-blue-500/20 active:scale-95 transition-all flex items-center justify-center gap-2"
                    >
                        ...
                        <div className="divide-y divide-slate-100">
                            {messages.map((message, idx) => (
                                <motion.div
                                    ...
                                    className={`p-5 sm:p-6 hover:bg-slate-50 cursor-pointer transition-colors relative group ${!message.isRead && activeTab === 'inbox' ? 'bg-blue-50/30' : ''}`}
                                >
                                    <div className="flex gap-4 sm:gap-6 items-start">
                                        <div className="shrink-0 pt-1">
                                            <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 ${!message.isRead && activeTab === 'inbox' ? 'bg-blue-600 border-blue-100 text-white' : 'bg-slate-50 border-slate-100 text-slate-400 group-hover:bg-white group-hover:border-blue-200 transition-colors'}`}>
                                        ...
                                        {!message.isRead && activeTab === 'inbox' && (
                                            <div className="absolute right-6 top-1/2 -translate-y-1/2 w-2 h-2 bg-blue-600 rounded-full shadow-sm shadow-blue-200"></div>
                                        )}
                                    </div>
                                </motion.div>
                            ))}
                        ...
                        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-100 p-4">
                            ...
                            >
                                <div className="flex justify-between items-center mb-8">
                                    <div>
                                        <p className="text-[10px] font-black uppercase tracking-widest text-blue-600 mb-1">Department Head</p>
                                ...
                                <form onSubmit={handleSendMessage} className="space-y-6">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <select
                                                ...
                                                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-slate-700 focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all font-bold text-sm appearance-none"
                                            >
                                                <option value="">Select Recipient</option>
                                                {divHeads.length > 0 && (
                                                    <optgroup label="Divisional Heads">
                                                        {divHeads.map(r => (
                                        ...
                                        <div>
                                            ...
                                            <select
                                                ...
                                                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-slate-700 focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all font-bold text-sm"
                                            >
                                                {MESSAGE_TYPES.map(t => (
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-slate-700 focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all font-bold text-sm resize-none"
                                        />
                                    </div>
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-slate-100">
                                        ...
                                        <button
                                            type="submit"
                                            className="order-1 sm:order-2 flex-1 px-6 py-4 bg-linear-to-br from-blue-600 to-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-blue-500/20 hover:shadow-blue-500/30 transition-all active:scale-95 flex items-center justify-center gap-2"
                                        >
                                ...
                                {selectedMessage.invoiceId && (
                                    <div className="mt-6 flex items-center justify-between p-4 bg-blue-50 rounded-2xl border border-blue-100">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="text-[10px] font-black uppercase tracking-widest text-blue-400 mb-0.5">Linked Invoice</p>
                                                <p className="text-sm font-bold text-blue-700">Ref: {selectedMessage.invoiceId}</p>
                                            </div>
                                        </div>
                                        <a
                                            href={`/dept-head/approval-queue?invoiceId=${selectedMessage.invoiceId}`}
                                            className="px-4 py-2 bg-white text-blue-600 border border-blue-100 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all shadow-sm"
                                        >
                                            View Task
                                        </a>
                                    </div>
