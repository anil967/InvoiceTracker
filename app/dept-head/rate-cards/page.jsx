...
export default function RateCardPage() {
    ...
    const fetchRatecards = async () => {
        try {
            ...
            const fetchUrl = `/api/dept-head/ratecards?${params}`;
            console.log('[DeptHead Page] Fetching:', fetchUrl);
            
            const res = await fetch(fetchUrl, { cache: 'no-store' });
            const data = await res.json();
            
            console.log('[DeptHead Page] Response status:', res.status);
            console.log('[DeptHead Page] Response data:', data);
            
            if (!res.ok) throw new Error(data.error);
            setRatecards(data.ratecards || []);
            
            console.log('[DeptHead Page] Ratecards set:', data.ratecards?.length || 0);
        } catch (err) {
            console.error('[DeptHead Page] Error:', err);
