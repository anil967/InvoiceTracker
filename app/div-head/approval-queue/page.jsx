            ...
            <AnimatePresence>
                {drawerOpen && (
                    <motion.div
                        ...
                        className="fixed inset-0 z-40 bg-black/20 backdrop-blur-sm flex items-end lg:relative lg:inset-auto lg:bg-transparent lg:backdrop-blur-none lg:flex-none"
                        onClick={(e) => { if (e.target === e.currentTarget) closeDrawer(); }}
                    >
                        <div className="w-full max-h-[92vh] lg:max-h-full lg:h-full lg:w-[520px] xl:w-[580px] bg-white lg:border-l border-slate-200 rounded-t-3xl lg:rounded-none overflow-y-auto flex flex-col shadow-2xl lg:shadow-none">
                            <div className="sticky top-0 z-10 bg-white px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                                <div>
                                    <h2 className="font-bold text-slate-800 text-base">Final Review</h2>
                            ...
                            {reviewLoading ? (
                                ...
                                <div className="flex-1 p-5 space-y-4">
                                    <Section title="Invoice Details" icon="FileText" accent="indigo">
                                    ...
                                    {pmUploadedDocs.length > 0 && (
                                        <Section title="PM Uploaded Documents" icon="FolderOpen" accent="violet">
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex gap-2">
                                                <select value={uploadForm.type} onChange={e => setUploadForm(f => ({ ...f, type: e.target.value }))}
                                                    className="flex-1 text-xs rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 font-medium text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-400">
                                    ...
                                    {/* Final Actions */}
                                    {(reviewInvoice.status === INVOICE_STATUS.PENDING_DIV_HEAD_REVIEW ||
                                        reviewInvoice.status === 'Pending Div Head Review') && (
                                            <div className="sticky bottom-0 bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-3">
                                                <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-3">Final Decision</h3>
                                                ...
                                                {actionType && (
                                                    <div className="space-y-2">
                                                        ...
                                                        <textarea
                                                            ...
                                                            className="w-full rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm font-medium text-slate-700 resize-none h-20 focus:outline-none focus:ring-2 focus:ring-indigo-400"
                                                    ...
                                                    {actionType ? (
                                                        ...
                                                        <>
                                                            ...
                                                            <button onClick={() => handleAction(reviewInvoice.id, 'APPROVE')}
                                                                disabled={!!processingId}
                                                                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-600 text-white hover:bg-indigo-700 font-bold text-sm transition-all disabled:opacity-50">
                                                                <Icon name="BadgeCheck" size={14} />
                                                                {processingId ? 'Processing...' : 'Final Approve'}
                                                            </button>
                                                        </>
                                                    )}
                                                </div>
