...
export default function RateCardPage() {
    const { user } = useAuth();
    const userRole = user ? getNormalizedRole(user) : 'PM';
    ...
    const fetchRatecards = async () => {
        try {
            ...
            const res = await fetch(`/api/pm/ratecards?${params}`, { cache: 'no-store' });
    ...
    return (
        <div className="px-4 sm:px-8 py-6 sm:py-8 min-h-screen">
            <PageHeader
                ...
                subtitle="View and verify vendor service rates"
                icon="Layers"
                accent="indigo"
                roleLabel={userRole}
            />
