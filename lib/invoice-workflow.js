...
export const WORKFLOW_TRANSITIONS = {
    ...
    [INVOICE_STATUS.PENDING_DEPT_HEAD_REVIEW]: [
        ...
        INVOICE_STATUS.MORE_INFO_NEEDED,           // Dept Head needs more info (sends to Vendor)
        INVOICE_STATUS.RECHECK_BY_DEPT_HEAD        // Dept Head re-check → sends back to PM
    ],

    // Re-check by Dept Head — invoice sent back to PM, awaiting PM re-review
    [INVOICE_STATUS.RECHECK_BY_DEPT_HEAD]: [
        INVOICE_STATUS.PENDING_DEPT_HEAD_REVIEW,   // PM re-approves → back to Dept Head
        INVOICE_STATUS.PM_REJECTED,                // PM rejects after re-check
        INVOICE_STATUS.MORE_INFO_NEEDED            // PM requests more info from vendor
    ],
    ...
    [INVOICE_STATUS.PENDING_DIV_HEAD_REVIEW]: [
        ...
        INVOICE_STATUS.MORE_INFO_NEEDED,           // Div Head needs more info (sends to Vendor)
        INVOICE_STATUS.RECHECK_BY_DIV_HEAD         // Div Head re-check → sends back to Dept Head
    ],

    // Re-check by Div Head — invoice sent back to Dept Head, awaiting Dept Head re-review
    [INVOICE_STATUS.RECHECK_BY_DIV_HEAD]: [
        INVOICE_STATUS.PENDING_DIV_HEAD_REVIEW,    // Dept Head re-approves → back to Div Head
        INVOICE_STATUS.DEPT_HEAD_REJECTED,          // Dept Head rejects
        INVOICE_STATUS.MORE_INFO_NEEDED,           // Dept Head requests more info from vendor
        INVOICE_STATUS.RECHECK_BY_DEPT_HEAD        // Dept Head also rechecks → sends back to PM
    ],
...
export const TRANSITION_PERMISSIONS = {
    ...
    [ROLES.PROJECT_MANAGER]: {
        ...
        // PM can act on invoices returned by Dept Head re-check
        [INVOICE_STATUS.RECHECK_BY_DEPT_HEAD]: [
            INVOICE_STATUS.PENDING_DEPT_HEAD_REVIEW,
            INVOICE_STATUS.PM_REJECTED,
            INVOICE_STATUS.MORE_INFO_NEEDED
    ...
    [ROLES.DEPARTMENT_HEAD]: {
        [INVOICE_STATUS.PENDING_DEPT_HEAD_REVIEW]: [
            ...
            INVOICE_STATUS.DEPT_HEAD_REJECTED,
            INVOICE_STATUS.MORE_INFO_NEEDED,
            INVOICE_STATUS.RECHECK_BY_DEPT_HEAD      // Re-check → sends back to PM
        ],
        // Dept Head can also act on invoices returned by Div Head
        [INVOICE_STATUS.RECHECK_BY_DIV_HEAD]: [
            INVOICE_STATUS.PENDING_DIV_HEAD_REVIEW,
            INVOICE_STATUS.DEPT_HEAD_REJECTED,
            INVOICE_STATUS.MORE_INFO_NEEDED,
    ...
    [ROLES.DIVISIONAL_HEAD]: {
        [INVOICE_STATUS.PENDING_DIV_HEAD_REVIEW]: [
            ...
            INVOICE_STATUS.DIV_HEAD_REJECTED,
            INVOICE_STATUS.MORE_INFO_NEEDED,
            INVOICE_STATUS.RECHECK_BY_DIV_HEAD       // Re-check → sends back to Dept Head
        ]
    },
